function build_model!(config, forecast_horizon, model_type::LSTMModel)
    @unpack model_type, cuda, verbose, resume, seed = config
    seed > 0 && Random.seed!(seed)

    # Get model hyperparameters
    @unpack input_dim, hidden_dim_resnet, rnn_input_dim, rnn_output_dim, z_dim = config
    @unpack latent_dim_z₀, latent_to_diffeq_dim, z₀_activation, general_activation, output_activation, init = config

    model_args = @dict(hidden_dim_resnet, rnn_input_dim, rnn_output_dim, z_dim,
                        latent_dim_z₀, latent_to_diffeq_dim, z₀_activation, general_activation,
    ...
    encoder_layers, decoder_layers = default_layers(model_type, input_dim; device=device, model_args...)
...
function init_training_state(model, config, model_type::LSTMModel)
...
@with_kw mutable struct TrainingState3
...
function save_data!(best_ps, best_ps_train, losses_hist, config, model_type::LSTMModel)
    merge!(config, @dict best_ps best_ps_train losses_hist)
    config = tostringdict(config)
    @unpack experiments_name, training_samples = config
    @unpack model_type, z_dim, seed = config
